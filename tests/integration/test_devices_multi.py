...
#   actual_devices = sorted(data['devices'], key=lambda x: x['id'])
...
def test_multiple_device_creation(base_url, session, devices):
  ...
  verify_devices(get_data['devices'], devices)
...
def test_devices_with_query_parameters(base_url, session, devices, parameter, value):
  ...
  verify_devices(get_data['devices'], expected_list, excluding)
