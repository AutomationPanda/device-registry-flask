...
class Device(db.Model):
  ...
  def validate_full(json_data):
    ...
    if missing_fields:
      raise ValidationError(f'request body has missing fields: {", ".join(missing_fields)}')

    invalid_fields = [key for key in json_data if key not in valid_fields]
    if invalid_fields:
      raise ValidationError(f'request body has invalid fields: {", ".join(invalid_fields)}')
  ...
  def patch_from_json(self, json_data):
    ...
    if invalid_keys:
      raise ValidationError(f'request body has invalid fields: {", ".join(invalid_keys)}')
